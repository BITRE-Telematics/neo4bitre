...
#' Turns a list of single column tibbles into a single tibble, "MATCH (s) return s.value, s.value2"
#'
#' @param res A result from call_neo4j()
#'
...
#' @examples
#' \dontrun{
#' "MATCH (s) return s.value, s.value2" %>% call_neo4j(con) %>% neo_tibble()
#' }

neo_tibble = function(res){
  tibnames = names(res)
  tib = suppressMessages(bind_cols(res))
  names(tib) = tibnames
  return(tib)
}

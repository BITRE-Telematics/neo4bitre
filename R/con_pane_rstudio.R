...
neo4j_actions <- function(url) {
  list(
    browser = list(
      icon = system.file("icons", "browserlogo.png", package = "neo4bitre"),
      callback = function() {
        browseURL(url)
      }
    ),
    help = list(
      icon = system.file("icons", "github.png", package = "neo4bitre"),
      callback = function() {
        browseURL("https://github.com/neo4j-rstats/neo4bitre")
      }
    )
  )
}
...
on_connection_opened <- function(con, url, auth) {
  ...
  if (!is.null(observer)) {
    observer$connectionOpened(
      ...
      icon = system.file("icons", "neologo.png", package = "neo4bitre"),
